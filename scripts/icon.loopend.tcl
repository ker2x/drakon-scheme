...
proc loopend.fit { tw th x y w h a b } {
	return [ list $x $y 60 20 $a $b ]
}
...
proc loopend.icons { text x y w h a b } {
	set h2 [ expr { $h / 1.5 } ]
	...
	set rect [ make_prim main polygon $coords "" $colors::line_fg $colors::for_bg $cdbox ]
	return [ list $rect ]
}
