...
proc commentin.fit { tw th x y w h a b } {
	set tmp [ action.fit $tw $th $x $y $w $h $a $b ]
	unpack $tmp x2 y2 w2 h2 a2 b2
	incr w2 8
	return [ list $x2 $y2 $w2 $h2 $a2 $b2 ]
}


proc commentin.icons { text x y w h a b } {
	set radius 12
	set number 8
	...
	set rect1 [ make_prim main rectangle $rect1_coords "" $colors::line_fg $colors::comment_bg $cdbox ]
	set screen [ make_prim screen polygon $coords "" $colors::line_fg $colors::action_bg $cdbox ]
