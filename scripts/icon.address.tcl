...
proc address.fit { tw th x y w h a b } {
...
proc address.icons { text x y w h a b } {
	set h2 [ expr { $h / 1.5 } ]
	...
	if { $b == 0 } {
		...
		set x1 [ expr { $x - $w / 2.0 } ]
		set x2 [ expr { $x + $w / 2.0 } ]
		set tri [ make_prim mark polygon [ list $x1 $m2 $x $top $x2 $m2 $x1 $m2 ] "" $colors::line_fg $colors::line_fg $cdbox ]
