...
proc branch.fit { tw th x y w h a b } {
	return [ address.fit $tw $th $x $y $w $h $a $b ]
}
...
proc branch.icons { text x y w h a b } {
	set h2 [ expr { $h / 1.5 } ]
	...
	set rect [ make_prim main polygon $coords "" $colors::line_fg $colors::action_bg $cdbox ]
	...
	if { $b == 0 } {
		...
		set x1 [ expr { $x - $w / 2.0 } ]
		set x2 [ expr { $x + $w / 2.0 } ]
		set tri [ make_prim mark polygon [ list $x1 $m2 $x $bottom $x2 $m2 $x1 $m2 ] "" $colors::line_fg $colors::line_fg $cdbox ]
