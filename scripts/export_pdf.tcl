...
proc prepare_export	 { } {
	...
	if { $prim_count == 0 } {
		tk_messageBox -parent . -message "This diagram is empty." -title Error
	return 0
	}
...
proc p.check { } {
	...
	if { ![ string is double $pr_margin ] } { return "Margin is not a number" }
	if { ![ string is double $pr_scale ] } { return "Scale is not a number" }

	if { $pr_margin < 0 } { return "Margin is negative." }
	set size [ p.page_size_units ]
	set width [ lindex $size 0 ]
	set height [ lindex $size 1 ]
	if { $width < 20 } { return "Margins are too wide." }
	if { $height < 20 } { return "Margins are too wide." }
	if { $pr_scale < 5 || $pr_scale > 500 } { return "Scale should be between 5 and 500." }
...
proc p.start_export { } {
	...
	if { $pr_file == "" } {
		p.complain "Output file name not chosen."
...
proc p.do_save_as_pdf { filename codepage paper_size orientation margin scale headers } {
	...
	unpack [ p.get_font ] font_filename font_size_base
...
proc p.print_prim_to_pdf { prim_id type fill outline text shift_x shift_y scale } {
	...
	if { $type == "text" } {
		...
		p.print_rectangle $prim_id $bg $outline $shift_x $shift_y $scale
	} elseif { $type == "line" } {
		p.print_line $prim_id $fill $shift_x $shift_y $scale
	} elseif { $type == "polygon" } {
		if { !$pr_colors && $fill != "#000000" } { set fill "#ffffff" }
...
proc p.reset_origin { } {
	...
	unpack $box left top right bottom
...
proc p.init { win data } {
	...
	ttk::labelframe $root.left.orientation -padding "3 3 3 3" -text "Orientation"
	ttk::radiobutton $root.left.orientation.portrait -text "Portrait" -variable export_pdf::pr_orientation -value portrait
	ttk::radiobutton $root.left.orientation.landscape -text "Landscape" -variable export_pdf::pr_orientation -value landscape
	...
	ttk::checkbutton $root.left.colors -variable export_pdf::pr_colors -text "Colors"
	pack $root.left.colors -padx 3 -pady 3

	ttk::button $root.left.one -text "Fit one page" -command export_pdf::p.fit_one
	pack $root.left.one -padx 3 -pady 3	 
	ttk::button $root.left.apply -text "Apply" -command export_pdf::p.apply_args
	...
	ttk::label $root.middle.label -text "Output file:"
	...
	ttk::button $root.bottom.cancel -text "Cancel" -command export_pdf::p.close
	pack $root.bottom.cancel -side right -padx 3 -pady 3

	ttk::button $root.bottom.export -text "Export" -command export_pdf::p.start_export
...
proc p.find_canvas_scale {	} {
	...
	unpack $paper_size paper_width paper_height
