...
proc zoom_see_all { canvas_width canvas_height } {
	...
	unpack [ find_diagram_rect $db $diagram_id ] left top width height
...
proc adjust_icon_sizes_current { } {
	...
	save_view
	start_action  "Adjust icon sizes for diagram"
...
proc adjust_icons_in_dia { diagram_id } {
	...
	set icons_to_adjust [ $db eval {
		select item_id
		from items
		where type not in ('vertical', 'horizontal', 'arrow') 
	...
	set actions [ $db eval {
		select item_id
		from items
		where type in ('action', 'insertion', 'loopstart', 'loopend')
			and diagram_id = :diagram_id
	} ]
...
proc add_friend_items { diagram_id name create_one createv destroyv dov undov } {
	...
	if { $name == "if" } {
		...
		set height 500
		set vy [ expr { $y - 40 } ]
		set dy [ expr { $vy + $height - 40 } ]
		
		set vertical2 [ mv::vertical.create $item2 $diagram_id $x $vy ]
		set vertical2 [ lreplace $vertical2 19 19 $height ]
		set vertical2 [ lreplace $vertical2 9 9 '' ]
		append_with_item $item2 $vertical2 create destroy do undo
		
		set c1 [ mv::address.create $item3 $diagram_id $x $dy ]
		set c1 [ lreplace $c1 9 9 '' ]
		append_with_item $item3 $c1 create destroy do undo
...
proc find_diagram_rect { db diagram_id } {
	...
	$db eval {
		...
		set box mv::$type.box
		unpack [ $box $x $y $w $h $a $b ] bleft btop bright bbottom
