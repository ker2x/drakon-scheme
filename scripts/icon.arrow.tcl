...
proc arrow.fit { tw th x y w h a b } {
...
proc make_line { role x0 y0 x1 y1 } {
	set coords [ list $x0 $y0 $x1 $y1 ]
	set cdbox [ add_handle_border $coords ]
	set line [ make_prim $role line $coords "" "" $colors::line_fg $cdbox ]
	return $line
}
...
proc arrow.lines { x y w h a b } {
	...
	if { $b == 0 } {
		set leftb [ expr { $x - $a } ]
		set leftt [ expr { $x - $w } ]
		set line1 [ make_line line1 $leftb $bottom $x $bottom ]
		set line2 [ make_line line2 $x $y $x $bottom  ]
		set line3 [ make_line line3 $leftt $y $x $y ]
	} else {
		set rightb [ expr { $x + $a } ]
		set rightt [ expr { $x + $w } ]
		set line1 [ make_line line1 $x $bottom $rightb $bottom ]
		set line2 [ make_line line2 $x $y $x $bottom ]
		set line3 [ make_line line3 $x $y $rightt $y ]
	}
	set head [ make_head $x $y $w $b ]
	return [ list $line1 $line2 $line3 $head ]
}

proc arrow.icons { text x y w h a b } {
	return {}
}
